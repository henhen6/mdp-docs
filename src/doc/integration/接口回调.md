...
```mermaid
flowchart LR
    A["调用方<br/>（携带 notifyUrl）"]
    subgraph MDP["MDP 平台"]
        B["api-service<br/>业务方法执行后调用<br/>NotifyInfoService#notify"]
        C["open-service<br/>加密 + HTTP 推送<br/>失败则定时重试"]
    end
    D["回调接收端（notifyUrl）<br/>验签 + 解密 + 处理业务"]
    A -->|"① 调用接口"| B
    B -->|"② 写入回调任务"| C
    C -.->|"③ POST 推送"| D
    D -.->|"④ 返回 code=0"| C
...
> 推送失败后由平台定时重试（梯度间隔），详见第 7 节[重试机制](#_7-重试机制)。
