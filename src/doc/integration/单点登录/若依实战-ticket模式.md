...
1. 联系MDP系统的开放平台管理员，由管理员在【控制台】创建应用。
...
2. 配置秘钥
   ...
   点击【秘钥】按钮，可以查看应用ID和应用秘钥，并配置事件订阅和公钥。

   :::: details 事件订阅

   事件订阅：MDP系统提前内置了一些“主数据”，在MDP平台增删改了这些主数据后，会通过事件广播给所有订阅了这个事件的应用，应用在接收到对应的事件后，需要自行编写代码接收和存储数据。

   - 平台在推送事件时，会将参数使用【平台私钥】进行签名。应用在接收到消息后，需要使用【平台公钥】对签名进行验签，验签成功后方可处理事件逻辑。

   - 应用在调用平台提供的接口时，需要将参数使用【应用私钥】进行签名。应用在接收到调用请求后，需要使用【应用公钥】对签名进行验签，验签成功后才会响应应用数据。
