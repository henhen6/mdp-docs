...
- 需要定时任务，只需要启动PowerJob和 worker-server（PowerJob执行器仅集成在worker-server，api-server不承担定时任务）。
...
项目名：worker-server。单体架构和微服务架构**都要启动**（PowerJob 执行器只集成在本服务）。主要提供以下功能：
...
上述组合不必靠改依赖实现：worker-server 同时具备开放接口、定时任务执行两种能力，只用其一时用开关裁剪另一个——只要开放接口就配 `powerjob.worker.enabled: false`，只要定时任务就配 `dubbo.enabled: false`。dev 环境出厂为 `dubbo.enabled=true`、`powerjob.worker.enabled=false`，详见[架构介绍 3.3 节](../info/架构介绍.md#_3-3-用两个开关裁剪-worker-server-的能力)。
...
3. 需要执行定时任务时
   - 后端服务仅需启动 PowerJob、worker-server（执行器只在worker-server，api-server不承担定时任务）
   - 前端项目无需启动
   - 中间件仅需mysql、redis，无需nacos

微服务版把 worker-server 当纯执行器用时（上面第 3 种），务必显式配 `dubbo.enabled: false`：否则它会与 api-server 同时注册成同一批开放接口的 provider，被 sop-gateway-server 轮询到。worker-server 的 test/prod 出厂值是两种能力全开，需要覆盖，详见[架构介绍 3.3 节](../info/架构介绍.md#_3-3-用两个开关裁剪-worker-server-的能力)。
