...
pnpm --filter @vben/web-workbench run dev:cloud       # 直接起指定应用
```

改 `apps/*/.env` 的 `VITE_GLOB_MODE=cloud` 也可以，但会影响 `pnpm dev:workbench` 等所有不带后缀的命令。详见[前端启动](../前端启动.md)与[前端配置](../../../config/前端配置.md)。
