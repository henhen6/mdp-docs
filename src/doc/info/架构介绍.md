...
```
md-open/open-facade/
├── open-facade          # AppFacade 接口定义（调用方只认这个）
├── open-boot-impl       # AppFacadeImpl → 注入 AppService，进程内直调
└── open-cloud-impl      # AppFacadeImpl → 注入 AppApi，走 Dubbo 远程调用
```

三个业务域（`md-workbench`、`md-console`、`md-open`）都是同样的 `{域}-boot-impl` / `{域}-cloud-impl` 成对结构：

```mermaid
flowchart LR
    F["AppFacade 接口<br/>调用方代码完全一致"]
    B["boot-impl 实现<br/>注入 AppService<br/>进程内方法调用"]
    C["cloud-impl 实现<br/>注入 AppApi<br/>Dubbo 远程调用"]
...
| 依赖 | worker-server | api-server | 说明 |
 ...
 | `{域}-cloud-impl`（Dubbo） | ❌ | ✅ | 通过远程调用访问其他域能力 |
 | `dubbo-zookeeper-curator5-spring-boot-starter` | ✅（dev） | ❌ | 由 pom profile 按环境切换注册中心 |
 | `dubbo-nacos-spring-boot-starter` | ✅（test/prod） | ✅ | api-server 固定用 Nacos |
