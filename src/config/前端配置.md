...
```mermaid
flowchart BT
    A[".env<br/>（基础，最先加载）"] --> B[".env.development<br/>或 .env.production<br/>（按模式叠加，同名覆盖）"]
    B --> C["自定义 .env.local 等<br/>（可选，本地调试用）"]
```
...
`VITE_PROXY` 是一段 JSON，按 `VITE_GLOB_MODE` 分成两套：
...
| 模式 | 目标 | 路径改写规则 | 说明 |
| ---- | ---- | ------------ | ---- |
| boot（单体） | `localhost:23455`（boot-server） | `/api/workbench/xxx` → `/workbench/xxx`（去掉 `/api` 和服务名段） | 单体版一个进程承接全部请求 |
| cloud（微服务） | `localhost:23450`（inner-gateway） | `/api/workbench/xxx` 原样转发 | 由内部网关按第一段路由到各服务 |

**前后端模式必须保持一致**：本地起单体版后端（boot-server）时，`VITE_GLOB_MODE=boot`；起微服务版（inner-gateway）时改为 `cloud`。切换方法：修改 `.env` 中的 `VITE_GLOB_MODE`，重启 `pnpm dev`。
...
> 生产环境 `VITE_PROXY` 不生效（它只是 Vite 开发服务器的能力），前后端联调走 nginx：把 `/api` 反代到 boot-server（单体）或 inner-gateway（微服务）即可。
...
workbench 是统一登录入口；console 通过 `VITE_GLOB_OPEN_PLATFORM_URL` 跳转开发者平台；它们之间通过 SSO 共享会话，无需重复登录。生产部署时确保该变量指向 web-open 的正式地址。
