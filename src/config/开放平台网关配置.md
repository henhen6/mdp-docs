...
开放平台网关（`sop-gateway-server`）是对外提供开放接口的唯一入口：第三方应用的所有请求（[接口调用](../doc/integration/接口调用.md)）都打到它的 `/api` 路径上，由它完成验签、token 校验后转发给 api-server。本文介绍它的专属配置。
...
这一段定义了开放接口的**协议参数名**（与[接口调用](../doc/integration/接口调用.md)中第三方传参一一对应）。除非平台级协议变更，**不建议修改**——改了参数名会导致所有已接入应用的请求解析失败。
