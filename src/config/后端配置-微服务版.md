...
| `dubbo.enabled` | boot-server 为 `false`（单体核心业务不引 Dubbo）；worker-server 承接开放接口时为 `true` | `true`（api-server、sop-gateway）；微服务版的 worker-server 应显式设 `false` | 5 个微服务在 Nacos `api-server.yml` 等；boot / worker / sop-gateway 在 jar 内 yml |
| Dubbo 注册中心客户端 | 随 Maven profile 切换（`dev` → ZooKeeper、`test` / `prod` → Nacos），链路两端行为一致 | 同一套 profile 机制下两端**不一致**：`api-server` 固定 Nacos，`sop-gateway-server` 在 `dev` 仍是 ZooKeeper，需手工换成 Nacos | `sop-gateway-server/pom.xml`、`worker-server/pom.xml` 的 profile 依赖 + 各自 `application-{env}.yml` 的 `dubbo.registry.address`，改法见[微服务版启动](../doc/start/backend/微服务版启动.md) |
| 端口 | boot 23455 / sop-gateway 23456 / worker 23457 | inner-gateway 23450 / console 23451 / open 23452 / workbench 23453 / api 23454 / sop-gateway 23456 / worker 23457 | 5 个微服务在 Nacos `{服务名}.yml`；boot / worker / sop-gateway 在 jar 内 `application.yml` |
