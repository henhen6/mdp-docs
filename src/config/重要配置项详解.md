...
本节单独列出，是因为里面藏着**两个能力开关**：`dubbo.enabled` 决定这个进程要不要提供开放接口，`powerjob.worker.enabled` 决定它要不要当定时任务执行器。两者配合起来裁剪 worker-server 的职责，原理见[架构介绍 §3.3](../doc/info/架构介绍.md)。

本节的"位置"直接写全路径，涉及的服务不止 boot-server：

### 17.1 dubbo.\* —— RPC 与开放接口

| 配置项 | 可选值 | 默认值 | 建议值 | 位置 |
| ------ | ------ | ------ | ------ | ---- |
| dubbo.enabled | true / false | **不配即 true**（`@ConditionalOnProperty` 的 `matchIfMissing=true`） | 与角色一致：boot-server 必须 false；微服务版 worker-server 建议 false | boot：主文件 `application.yml:419` = false；worker：`application-dev.yml:17` = true；api：Nacos `api-server.yml` = true |
| dubbo.protocol.name | dubbo / rest / … | dubbo | 保持 dubbo | worker 主文件、各服务 yml |
| dubbo.protocol.port | 端口 / `-1`（随机可用端口） | - | `-1`，交给 Dubbo 选端口 | 同上 |
| dubbo.application.qos-enable | true / false | 未配时 Dubbo 默认开 | **false**（否则 22222 端口会随进程数冲突） | 同上 |
| dubbo.consumer.timeout | 毫秒 | Dubbo 默认 1000 | MDP 统一 30000 | 同上 |
| dubbo.consumer.retries | 次数 | Dubbo 默认 2 | **必须 0**——超时重试会让服务端重复处理同一请求 | 同上 |
| dubbo.consumer.check | true / false | 未配时 Dubbo 默认 true | false（启动不强制要求 provider 已存在） | 同上 |
| dubbo.provider.validation | jvalidationNew / true / false | - | `jvalidationNew`（JSR-303 校验） | 同上 |
| dubbo.provider.filter | 过滤器名，逗号分隔 | - | 保持 `globalExceptionFilter,exception`，去掉会导致远程异常丢失 | 同上 |
| dubbo.registry.address | `zookeeper://ip:2181` / `nacos://user:pwd@ip:port?namespace=xxx` | - | dev 可用 zookeeper（内置 zookeeper-server 或自建）；**test/prod 必须 nacos** | worker：`application-dev.yml:20` zk；`application-test.yml`、`application-prod.yml` nacos |
| dubbo.registry.check | true / false | 未配时 Dubbo 默认 true | false（注册中心抖动不阻塞启动） | 同上 |

> `dubbo.enabled=false` 是**一箭双雕**：Dubbo 自身不再导出 provider，MDP 的 `SopAutoConfiguration`（同一条件属性）也不再扫描 `@DubboService` 接口上报给开放平台，不会出现"注册了但接口列表为空"的半开状态。

### 17.2 powerjob.worker.\* —— 定时任务执行器

绑定类：`PowerJobProperties`（`mdp-base/md-powerjob-worker-spring-boot-starter`，即 PowerJob 官方 starter 的 MDP 适配版）。**全仓只有 worker-server 引入该 starter**，所以这些配置只对它有意义。

| 配置项 | 可选值 | 代码默认值 | MDP 实际值 / 建议 | 位置 |
| ------ | ------ | ---------- | ----------------- | ---- |
| powerjob.worker.enabled | true / false | true（`:104`） | 主文件 `:435` 写 true；**`application-dev.yml:25` 覆盖为 false**（注释「需要使用 定时任务 功能时开启」） | worker：主文件 + env |
| powerjob.worker.app-name | 任意字符串，无默认 | - | `MDP`；**必须与 PowerJob 控制台里已注册的应用名完全一致**，否则 worker 注册被拒 | worker：主文件 `:437` |
| powerjob.worker.server-address | `ip:port`，多值逗号分隔，无默认 | - | `127.0.0.1:17700`；填的是**调度中心**地址，不是 worker 自己的端口。`enabled=true` 且该项为空会直接启动失败（`PowerJobAutoConfiguration.java:37` fail fast，报错原文就提示改用 `enabled=false`） | worker：主文件 `:439` |
| powerjob.worker.protocol | `AKKA` / `HTTP` | **AKKA**（`:138`） | MDP 显式设为 `http`，保持不动；注意 server 与 worker 两端都要放通端口 | worker：主文件 `:441` |
| powerjob.worker.store-strategy | `DISK` / `DB` | DISK（`:142`） | 保持 DISK（Map/MapReduce 类任务走磁盘，避免 OOM） | worker：主文件 `:443` |
| powerjob.worker.max-result-length | 字节数 | 8192（`:147`） | MDP 收紧为 4096，超出部分被截断 | worker：主文件 `:445` |
| powerjob.worker.max-appended-wf-context-length | 字节数 | 8192（`:157`） | MDP 4096，超出直接丢弃 | worker：主文件 `:447` |
| powerjob.worker.max-lightweight-task-num | 并发数 | 1024（`:163`） | 同默认；执行器 CPU 核数少时可调低 | worker：主文件 `:449` |
| powerjob.worker.max-heavyweight-task-num | 并发数 | 64（`:167`） | 同默认。**注意 yml 里写的是驼峰 `maxHeavyweightTaskNum`**（`:451`），Spring 宽松绑定能识别，但与其他项风格不一致 | worker：主文件 `:451` |
| powerjob.worker.allow-lazy-connect-server | true / false | false（`:152`） | 保持 false；它只用于"本地没有调度中心也要启动"的调试场景，**不能当作关闭执行器的替代手段** | 未配置（取默认） |
| powerjob.worker.health-report-interval | 秒 | 10（`:171`） | 保持默认 | 未配置 |
| powerjob.worker.port / akka-port | 端口 | `port` 为空时回落到 `akka-port`，后者 ≤ 0 则**随机端口**（`PowerJobAutoConfiguration.java:48-56`） | 单实例随机即可；同一台机器跑多个 worker 实例时显式指定，方便放通防火墙 | 未配置 |
